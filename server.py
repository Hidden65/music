...
Y hacer de tu cuerpo todo un manuscrito''',
    ...
    def handle_api_audio(self, query_string: str) -> None:
        """Resolve a direct audio stream URL for a YouTube video via yt-dlp.

        Returns JSON: { url: string, expiresIn: seconds }
        """
        params = urllib.parse.parse_qs(query_string or '')
        ...
        try:
            cmd = f"yt-dlp -f bestaudio[ext=m4a]/bestaudio --no-warnings --get-url {shlex.quote(video_url)}"
            proc = subprocess.run(cmd, shell=True, capture_output=True, text=True, timeout=15)
            if proc.returncode != 0:
                self.send_json_response({'error': 'Failed to resolve audio URL', 'detail': (proc.stderr or '').strip()}, 502)
                return
            url = (proc.stdout or '').strip().splitlines()[-1] if proc.stdout else ''
            if not url or not url.startswith('http'):
                self.send_json_response({'error': 'Invalid URL from resolver'}, 502)
                return
            self.send_json_response({'url': url, 'expiresIn': 900})
        except subprocess.TimeoutExpired:
            self.send_json_response({'error': 'Resolver timeout'}, 504)
        except FileNotFoundError:
            self.send_json_response({'error': 'yt-dlp not installed on server'}, 500)
        except Exception as e:
            self.send_json_response({'error': 'Unexpected error', 'detail': str(e)}, 500)
